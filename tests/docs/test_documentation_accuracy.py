...
class DocumentationValidator:
    ...
    def extract_api_endpoints_from_code(self) -> Dict[str, List[Tuple[str, str]]]:
        ...
        for route_file in routes_dir.glob("*.py"):
            ...
            route_pattern = r'@router\.(get|post|put|delete|patch|api_route)\(\s*"([^"]+)"'
    ...
    def extract_cli_commands_from_code(self) -> Set[str]:
        ...
        for cli_file in cli_dir.rglob("*.py"):
            ...
            function_command_pattern = r"@app\.command\([^)]*\)\s*def\s+([a-zA-Z_][a-zA-Z0-9_]*)"
    ...
    def extract_workflow_names_from_code(self) -> Set[str]:
        ...
        flows_dir = (
            self.ingenious_dir / "services" / "chat_services" / "multi_agent" / "conversation_flows"
        )
        if flows_dir.exists():
