  ...
  <div class="bg-white rounded-lg border border-gray-200 overflow-hidden">
    ...
    >
      <div class="flex items-center gap-4">
        ...
        <div>
          <p class="text-sm font-medium text-mine">{{ result.submissionName }}</p>
          <p v-if="result.submissionAuthor" class="text-xs text-taupe">{{ result.submissionAuthor }}</p>
        </div>
      </div>
      <div class="flex items-center gap-6">
        ...
        <svg
          :class="[
            'w-5 h-5 text-taupe transition-transform',
            { 'rotate-180': expanded }
          ]"
          fill="none"
          stroke="currentColor"
    ...
    <div v-if="expanded" class="border-t border-gray-100 p-5 bg-gray-50">
      <div v-if="result.criterionResults.length" class="grid gap-3 mb-4" :style="{ gridTemplateColumns: `repeat(${Math.min(result.criterionResults.length, 6)}, 1fr)` }">
        <div
          v-for="cr in result.criterionResults"
          :key="cr.criterionId"
          class="text-center"
        >
          <p class="text-sm font-semibold text-mine">{{ cr.score.toFixed(1) }}</p>
          <p class="text-xs text-taupe">{{ getCriterionName(cr.criterionId) }}</p>
        </div>
      </div>
      <div class="text-sm text-taupe leading-relaxed">
        <p>
          <strong class="text-mine">Summary:</strong> {{ result.summary }}
        </p>
      </div>
    </div>
  </div>
