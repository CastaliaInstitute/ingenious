...
<template>
  <div class="bg-white rounded-lg border border-gray-200 p-4 flex items-center justify-between">
    <div class="flex items-center gap-4">
      ...
      <div>
        <p class="text-sm font-medium text-mine">{{ submission.name }}</p>
        <p class="text-xs text-taupe">
          {{ submission.fileName }} &middot; {{ formatFileSize(submission.fileSize) }} &middot; {{ formatDate(submission.uploadedAt) }}
        </p>
      </div>
    </div>
    <button
